...
import {
  ArrowLeft, Edit, Lightbulb, Paintbrush, Refrigerator,
  ChevronRight, StickyNote, Plug, Shield, Home,
} from "lucide-react";
import type { Room, Appliance, Json } from "@/types/database";
...
export function RoomDetailClient({ propertyId, propertyName, room, appliances }: RoomDetailClientProps) {
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-stone capitalize mt-1">{room.type || "Room"} · {propertyName}</p>
        </div>
      ...
      {/* Room photo */}
      <PlaceholderImage type={room.type || "room"} seed={room.id + "-detail"} className="h-64 sm:h-80 w-full rounded-2xl mb-8" label={room.name} />
        ...
        <Card className="bg-white border-clay/20">
          ...
          <CardContent>
            {fixtures.length > 0 ? (
              ...
              <p className="text-sm text-stone">No fixtures documented yet.</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="bg-white border-clay/20">
          ...
          <CardContent>
            {features.length > 0 ? (
              ...
              <p className="text-sm text-stone">No features documented yet.</p>
            )}
          </CardContent>
        </Card>
      </div>
